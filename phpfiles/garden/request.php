...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["equiptype"])) {
    ...
    {
      $query = "SELECT max(reqid) as id FROM request";
      $result = mysqli_query($db, $query) or die(mysqli_error($db));
      $id = mysqli_fetch_assoc($result);
      $id = $id['id'];
      $id = $id + 1;

      $query = "INSERT INTO request(reqid, equiptypeid, empid) VALUES($id, '$equiptype', $empid)";
      // echo $query;
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <?php
    $query = "SELECT * FROM request NATURAL JOIN equiptype WHERE empid = $empid AND request.status <> 'ACCEPTED'";
