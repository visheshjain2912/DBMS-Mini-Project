...
{
  // echo $_POST["repaircost"];
  // echo $_POST["equipid"];
  if(empty($_POST["equipid"]) || empty($_POST["repaircost"]))
  {
    $emptyerror = true;
  }
  else
  {
    ...
    $query = "SELECT * FROM equipments WHERE equipid = '$equipid' AND owner LIKE 'V-%'";
    ...
    else
    {
      ...
      $today = date('Y-m-d');
      $query = "UPDATE repair SET repaircost = $cost WHERE equipid = '$equipid' and vendorid = '$owner' and returndate = '2001-01-01'";
      mysqli_query($db,$query) or die(mysqli_error($db));

      $query = "UPDATE repair SET returndate = '$today' WHERE equipid = '$equipid' and vendorid = '$owner' and returndate = '2001-01-01'";
      mysqli_query($db,$query) or die(mysqli_error($db));
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <?php
    $query = "SELECT * FROM equipments NATURAL JOIN equiptype NATURAL JOIN repair NATURAL JOIN vendor WHERE owner LIKE 'V-%'";
    // echo $query;
...
<!-- form for booking details -->
  <form action="repairreturn.php" method="POST">
    <?php
      if ($emptyerror) {
      ...
      if ($invaliderror) {
        echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                  Invalid Equipment ID or Equipment not in REPAIR!
                  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
              </div>';
      }
      if ($success) {
      ...
      <div>
        ...
        <input type="number" class="form-control" name="repaircost" id="repaircost">
      </div>
      <br>
      <div class="subbutton">
