...
<?php require('./nav.php'); ?>
  ...
  <br>
  <h2 style="text-align: center;">Monthly Food Billings : </h2>

  <form action="foodbilllings.php" method="POST">
    <?php
    if ($emptyerror) {
  ...
  <?php
      if(!$emptyerror && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["month"]))
      {
        ...
        $query = "SELECT bookingID FROM booking WHERE arrival <= '$end' AND departure > '$start' AND foodservices = 1";
        ...
        echo '<table class="table">
                <thead>
                  <tr>
                    ...
                    <th scope="col">Food Bill Amount</th>
                  </tr>
                </thead>
                <tbody>';
        ...
        while($row = mysqli_fetch_assoc($bookingids))
        {
          ...
          $query = "SELECT roomid FROM room NATURAL JOIN isBooked WHERE bookingid = '$id'";
          ...
          $foodbill = $numguest*$numdays*300;
          echo '<td>Rs. '.$foodbill.'</td>';
          echo '</tr>';
          $expense = $expense + $foodbill;
        }
        echo '<tr><td></td><td></td><td></td><td style="text-align:right;">Total : </td><td>Rs. '.$expense.'</td></tr>';
        echo '</tbody>
        </table>';
      }
  ?>
